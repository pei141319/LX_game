...
#define WIFI_SSID "wujietanzhi"    //wifi名称
#define WIFI_PASSWORD "12345678"  //wifi密码
//MQTT三元组
#define ClientId "wujie"
#define Username "bkrc"
#define Password "88888888"

#define MQTT_Address "115.28.209.116"
#define MQTT_Port 1883
#define Iot_link_MQTT_Topic_Report "device/693d0159a895525f/up"
#define Iot_link_MQTT_Topic_Commands "device/693d0159a895525f/down"
...
void protocolJSON(unsigned char JsonData[]) {
  ...
  root["sign"] = "693d0159a895525f";
  root["type"] = 3;

  JsonObject& temphumi = data.createNestedObject("actuator");            //温湿度传感器模块标识
  JsonObject& gas = data.createNestedObject("gas"); 
  JsonObject& Dust = data.createNestedObject("Dust");   
  sprintf(printf_buf, "%d", 0);

  temphumi["temperature"] = printf_buf;  //温度模型标识
  temphumi["humidity"] = printf_buf;  //湿度模型标识
  gas["mashgas"]=printf_buf; 
  Dust["dust"]=printf_buf;

  if (JsonData[0] == 0x55) {
    if (JsonData[1] == 0xAA) {
 sprintf(total_data, "%d", JsonData[2]);
      temphumi["temperature"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);

      sprintf(total_data, "%d", JsonData[3]);
      temphumi["humidity"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);

      sprintf(total_data, "%d", JsonData[4]);
      gas["mashgas"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);
           
      sprintf(total_data, "%d", JsonData[5]);
      Dust["dust"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      delay(50);
    }
  }
